use cosmic::widget::text_editor::Status;

pub fn text_editor<'a>() -> cosmic::theme::iced::TextEditor<'a> {
    cosmic::theme::iced::TextEditor::Custom(Box::new(style))
}

fn style(theme: &cosmic::Theme, status: Status) -> cosmic::iced_widget::text_editor::Style {
    ...
    match status {
        Status::Active | Status::Disabled => cosmic::iced_widget::text_editor::Style {
        ...
        | Status::Focused { is_hovered: false } => cosmic::iced_widget::text_editor::Style {
            background: background.into(),
            border: cosmic::iced::Border {
                radius: cosmic.corner_radii.radius_s.into(),
                width: 1.0,
