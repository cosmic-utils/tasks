...
impl Content {
    ...
    pub fn list_view(&self) -> Element<Message> {
        ...
        let items = self.tasks.iter().map(|item| {
            let row = widget::row::with_children(vec![
                widget::checkbox(
                    "",
                    item.status == Status::Completed,
                    |value| Message::Complete(item.id.clone(), value),
                ).into(),
                widget::text(item.title.clone()).width(Length::Fill).into(),
                widget::button::icon(widget::icon::from_name("user-trash-full-symbolic").size(16).handle()).on_press(Message::Delete(item.id.clone())).into(),
            ])
        ...
        widget::container(
            widget::column::with_children(
                vec![
                    widget::column::with_children(items).spacing(space_xxs).height(Length::Fill).into(),
                    row(
                        vec![
                            ...
                            widget::button::icon(widget::icon::from_name("list-add-symbolic").size(16).handle()).on_press(Message::AddTask).into(),
                        ]
                    ).spacing(space_xxs).align_items(Alignment::Center).into()
                ]
            )
        )
            .height(Length::Fill)
        ...
        match message {
            ...
            Message::Complete(id, complete) => {
                ...
                if let Some(task) = task {
                    task.status = if complete { Status::Completed } else { Status::NotStarted };
...
fn button_style(selected: bool, accent: bool) -> theme::Button {
    ...
    theme::Button::Custom {
        active: Box::new(move |focused, theme| button_appearance(theme, selected, focused, accent, false)),
