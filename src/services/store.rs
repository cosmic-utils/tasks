...
impl ListStore<'_> {
    pub fn get(&self, list_id: Uuid) -> Result<List> {
        self.load_all()?
            .into_iter()
            .find(|l| l.id == list_id)
            .ok_or(AppError::ListNotFound(list_id))
    }
    ...
    pub fn delete(&self, list_id: Uuid) -> Result<()> {
        ...
        if lists.len() == before {
            return Err(AppError::ListNotFound(list_id));
        }
...
impl TaskStore<'_> {
    pub fn get(&self, task_id: Uuid) -> Result<Task> {
        ...
        let content = fs::read_to_string(&path).map_err(|_| AppError::TaskNotFound(task_id))?;
        Ok(ron::from_str(&content)?)
    }
    ...
    pub fn load_all(&self) -> Result<Vec<Task>> {
        ...
        if !list_dir.exists() {
            return Err(AppError::ListNotFound(self.list_id));
        }
    ...
    pub fn delete(&self, task_id: Uuid) -> Result<()> {
        let path = self.store.task_path(self.list_id, task_id);
        fs::remove_file(&path).map_err(|_| AppError::TaskNotFound(task_id))
    }
